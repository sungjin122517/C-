 ...
 **/
int is_sunflower(char farm[][10], int x, int y, int M, int N)
{
    int identify = 0;
    
    if (x>=0 && y>=0 && x<=N && y<=M) {
        if (farm[y][x] == '.')
            identify = 0;   //0 for sunflower
        else if (farm[y][x] == '#')
            identify = 1;   //1 for nut
        else if (farm[y][x] == '+')
            identify = 2;   //2 for damaged nut
        else if (farm[y][x] == 'x')
            identify = 3;   //3 for zombie
...
void infect_farm(char farm[][10], int x, int y, int t, int M, int N, int T){
    ...
    int identify2 = is_sunflower(farm, x, y, M, N);
